...
with st.sidebar:
    ...
    pilihan = st.selectbox(
        ...
        ["Persegi", "Persegi Panjang", "Lingkaran", "Segitiga", "Tabung"]
    )
...
match pilihan:
    ...
    case "Tabung":
        st.title("Tabung")
        st.markdown("Menghitung `volume` dan `luas permukaan` tabung")
        ...
        if st.button("Hitung", type="primary"):
            ...
            with col1:
                st.metric("Volume", value=f"{volume:.2f}", border=True)

            with col2:
                st.metric("Luas Permukaan", value=f"{luas_permukaan:.2f}", border=True)
